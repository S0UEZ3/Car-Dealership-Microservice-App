...
public class RabbitMQConfig {
    ...
    public Binding bindingApproval() {
        return BindingBuilder.bind(orderApprovalQueue())
                .to(exchange()).with(ORDER_APPROVAL_QUEUE);
    }

    @Bean
    public Binding bindingApproved() {
        return BindingBuilder.bind(orderApprovedQueue())
                .to(exchange()).with(ORDER_APPROVED_QUEUE);
    }

    @Bean
    public Binding bindingRejected() {
        return BindingBuilder.bind(orderRejectedQueue())
                .to(exchange()).with(ORDER_REJECTED_QUEUE);
    }
